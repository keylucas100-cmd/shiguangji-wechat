...
function normalizeFoodName(name) {
  return String(name || '')
    .replace(/\s/g, '')
    .replace(/[，,。.!！?？、]/g, '')
    .replace(/^(我|帮我|给我|请|想|要|需要|今天|昨天|昨日|明天|刚刚|刚才|买了|买|购买了|购买|采购了|采购|添加|录入|记录)+/, '')
}
...
function inferCategoryByFoodName(name) {
  ...
  const knownFood = knownFoods.find(food => cleanName === food.name || cleanName.includes(food.name))
  if (knownFood) return knownFood.category

  if (/(鱼|虾|蟹|贝|蛤|蚬|螺|鱿鱼|章鱼|墨鱼|带鱼|三文鱼|鳕鱼|鲈鱼|黄鱼|鲫鱼|鲤鱼|鲳鱼|扇贝|生蚝|牡蛎|海鲜)/.test(cleanName)) {
    return '海鲜'
  }

  if (cleanName.endsWith('肉')) return '肉类'

  if (/(排骨|鸡翅|鸡腿|鸡爪|鸭腿|鸭脖|牛腩|牛排|肥牛|火腿|香肠|培根)/.test(cleanName)) {
    return '肉类'
  }
...
function parseNumberText(value) {
  const text = String(value || '').trim().replace(/两/g, '二')
  ...
  return Object.prototype.hasOwnProperty.call(numberMap, text) ? numberMap[text] : NaN
}

function parseFoodInfo(text) {
  const knownFood = knownFoods
  ...
  return genericCategory ? { name: genericFoodName, category: genericCategory } : {}
}

function parseQuantityInfo(text) {
  const halfJinMatch = text.match(/(\d+(?:\.\d+)?|[一二两三四五六七八九十]+)\s*斤半/)
  if (halfJinMatch) {
    const quantity = parseNumberText(halfJinMatch[1])
    return Number.isNaN(quantity) ? {} : { quantity: String(quantity + 0.5), unit: '斤' }
  }

  const match = text.match(/(\d+(?:\.\d+)?|[一二两三四五六七八九十半]+)\s*(公斤|千克|kg|KG|克|g|斤|盒|袋|瓶)/)
  if (!match) return {}
...
function parseShelfLifeDays(text) {
  const match = text.match(/保质期\s*(\d+(?:\.\d+)?|[一二两三四五六七八九十]+)\s*(天|日|个月|月|年)/)
  if (!match) return ''
...
function parseRelativeDate(text, keywords) {
  ...
  for (const day of dayOptions) {
    for (const keyword of keywords) {
      if (cleanText.includes(`${day.text}${keyword}`) || cleanText.includes(`${keyword}${day.text}`)) {
        return day.value
      }
    }
  }
...
Page({
  ...
  onLoad(query) {
    if (query.id) {
      ...
      if (item) {
        ...
        const idx = Math.max(0, categories.findIndex(c => c === item.category))
        this.setData({
          form: {
            ...createDefaultForm(),
            ...item,
  ...
  applyVoiceText(text) {
    ...
    if (nextForm.purchaseDate && nextForm.productionDate && nextForm.productionDate > nextForm.purchaseDate) {
    ...
    if (nextForm.productionDate && nextForm.shelfLifeDays) {
      const expireDate = calculateExpireDate(nextForm.productionDate, nextForm.shelfLifeDays)
